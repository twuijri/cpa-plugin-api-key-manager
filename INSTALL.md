...
## اكتشاف الموديلات المباشر (alpha.7)

بعد تحديث الإضافة وإعادة تشغيل حاوية CPA، أبقِ عنوان هرمز `http://cli-proxy-api:8317/v1` واستخدم مفتاح `mf_` صالحًا. لا تحتاج صورة بروكسي معدلة أو تحويل Caddy. يعرض `/v1/models` قائمة البروكسي العامة، وقد تظهر موديلات غير مسموحة للمفتاح؛ طلب استخدامها يظل مرفوضًا. المسارات المسماة الخاصة بالإضافة ليست بالضرورة ضمن قائمة CPA العامة ويمكن إدخالها يدويًا. المفاتيح المعطلة والمنتهية وغير الصحيحة مرفوضة. المفاتيح الأصلية لا تتغير.

## قائمة مفلترة اختيارية عبر Caddy (من alpha.6)
...
هذا تحويل داخلي وليس redirect؛ لا تُرسَل المفاتيح إلى جهة جديدة. يخص GET /v1/models مع Bearer mf_ فقط. المفاتيح الأصلية وطلبات المحادثة تبقى في مسارات CPA الأصلية. الاتصال المباشر بمنفذ CPA لا يحصل على هذا التحويل: منذ alpha.7 يعرض القائمة العامة للمفتاح الصالح. إذا أزيلت الإضافة، يفشل جلب القائمة المفلترة.
...
### التحديث دون إعادة تشغيل (alpha.8)

CPA يدعم استبدال الإضافة أثناء التشغيل. منذ alpha.8 يحرر مفتاح قفل البيانات في مرحلة quiesce بعد منع الطلبات الجديدة وانتظار العمليات وإغلاق جسور البث؛ فتستطيع النسخة الجديدة فتح الحالة بأمان. عند فشل تحميلها تُعيد القديمة فتح الملف وقراءة أحدث بياناته. لا يتم حذف القفل أو استخدام Docker socket أو إيقاف عملية البروكسي.

**للانتقال من alpha.7 أو أقدم:** حدّث الإضافة ثم أعد تشغيل حاوية CPA مرة أخيرة؛ النسخة القديمة لا تحتوي إصلاح تسليم القفل. التحديثات اللاحقة من alpha.8 إلى نسخة تدعم نفس التسليم لا تحتاج restart بسبب هذا القفل. اختُبر الاستبدال والتراجع على CPA الرسمي v7.2.146 محليًا؛ تحقّق من رقم النسخة المحملة في لوحتك بعد التحديث. التحديث قد ينهي بثًا جاريًا أو يرفض طلبًا أثناء الانتقال؛ نفّذه في وقت هدوء. لا يلزم تحديث صورة Docker أو إعادة تشغيل خدمة Docker أو السيرفر.
...
```text
https://raw.githubusercontent.com/twuijri/cpa-plugin-api-key-manager/main/registry.json
```
...
cd miftah-download
gh release download v0.1.2 --repo twuijri/cpa-plugin-api-key-manager \
  --pattern 'miftah_0.1.2_linux_amd64.zip' --pattern checksums.txt
sha256sum -c checksums.txt
unzip miftah_0.1.2_linux_amd64.zip
```
